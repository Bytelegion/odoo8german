...
class bridge_backbone(models.Model):
	...
	def _get_virtual_product_id(self, cr, uid, data):
		...
		if data['name'].startswith('S'):
			erp_product_id = ir_values.get_default(cr, uid, 'product.product', 'mob_delivery_product', False)
		if data['name'].startswith('D'):
			erp_product_id = ir_values.get_default(cr, uid, 'product.product', 'mob_discount_product', False)
		if data['name'].startswith('V'):
			erp_product_id = ir_values.get_default(cr, uid, 'product.product', 'mob_coupon_product', False)
		if not erp_product_id:
			...
			ir_values.set_default(cr, uid, 'product.product', object_name, erp_product_id, False)
			cr.commit()
	...
	def create_order_invoice(self, cr, uid, data, context=None):
		...
		if data.get('order_id'):
			...
			if context.has_key('instance_id'):
				...
				elif sale_order_details.get('order_policy') == 'picking':
					invoice_id = self.pool.get('sale.order').action_invoice_create(cr, uid, data.get('order_id'))
				else:
			...
			if order_obj.shipped:
				self.pool.get('sale.order').write(cr, uid, order_id, {'state':'done'})
		return invoice_id
