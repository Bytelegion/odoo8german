...
class magento_website(osv.osv):
	...
	def _get_website(self, cr, uid, website, context=None):
		...
		if websites:
			...
			self.write(cr, uid, website_id, website_dict)
		else:
...
class magento_store(osv.osv):
	...
	def _get_store_group(self, cr, uid, group, website, context=None):
		...
		website_id = self.pool.get('magento.website')._get_website(cr, uid, website, context)
		group_dict = {
				'name':group['name'],
		...
		if groups:
			group_id = groups[0]
			self.write(cr, uid, group_id, group_dict)
		else:
...
class magento_store_view(osv.osv):
	...
	def _get_store_view(self, cr, uid, store, context=None):
		...
		group_id = self.pool.get('magento.store')._get_store_group(cr, uid, store['group'], store['website'], context)		
		view_dict = {
							'name':store['name'],
							'code':store['code'],
		...
		if views:
			view_id = views[0]
			self.write(cr, uid, view_id, view_dict)			
		else:
...
class magento_configure(osv.osv):
	...
	def fetch_connection_info(self, cr, uid, vals, context=None):
		"""
			Called by Xmlrpc from Magento
		"""
		if vals.has_key('magento_url'):
			active_connection_ids = self.search(cr, uid, [('active','=',True)])			
			module_id = self.pool.get('ir.module.module').search(cr, uid, [("name","=","mob_multi_instance"),("state","=","installed")], context=context)
			if module_id:
