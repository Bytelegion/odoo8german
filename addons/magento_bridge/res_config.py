...
class mob_config_settings(osv.osv_memory):
    ...
    def set_default_fields(self, cr, uid, ids, context=None):
        ...
        ir_values.set_default(cr, uid, 'product.product', 'mob_delivery_product',
            config.mob_delivery_product and config.mob_delivery_product.id or False, False)
        ir_values.set_default(cr, uid, 'product.product', 'mob_discount_product',
            config.mob_discount_product and config.mob_discount_product.id or False, False)
        ir_values.set_default(cr, uid, 'product.product', 'mob_coupon_product',
            config.mob_coupon_product and config.mob_coupon_product.id or False, False)
        return True
    ...
    def get_default_fields(self, cr, uid, ids, context=None):
        ...
        ir_values = self.pool.get('ir.values')
        config = self.browse(cr, uid, ids[0], context)
        mob_delivery_product = ir_values.get_default(cr, uid, 'product.product', 'mob_delivery_product', False)
        mob_discount_product = ir_values.get_default(cr, uid, 'product.product', 'mob_discount_product', False)
        mob_coupon_product = ir_values.get_default(cr, uid, 'product.product', 'mob_coupon_product', False)
